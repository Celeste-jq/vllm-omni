...
def test_direct_pipeline_generate_forwards_expected_kwargs(tmp_path: Path):
    class _FakeTTSModel:
        sample_rate = 24000
    ...
    pipeline = _DirectVoxCPMPipeline(tts_model)
    ...
    assert len(tts_model.calls) == 1
    assert tts_model.calls[0]["target_text"] == "hello world"
    assert tts_model.calls[0]["prompt_wav_path"] == str(prompt_wav)
    assert tts_model.calls[0]["prompt_text"] == "ref"
    assert tts_model.calls[0]["cfg_value"] == 3.0
    assert tts_model.calls[0]["inference_timesteps"] == 12
