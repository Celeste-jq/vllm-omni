...
"""
A/B benchmark for WAN fast layernorm.

This script runs text_to_video.py in two cases:
1) fast_on  : VLLM_OMNI_WAN_FAST_LAYERNORM_ENABLE=1
2) fast_off : VLLM_OMNI_WAN_FAST_LAYERNORM_ENABLE=0

It records wall time, parsed generation time, and whether key fast-layernorm
logs appear in each run.

Example:
  python examples/offline_inference/text_to_video/benchmark_wan_fast_layernorm_ab.py \
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="A/B benchmark for WAN fast layernorm on/off.")
    ...
    parser.add_argument("--output-dir", default="outputs_wan_fast_layernorm_ab", help="Output directory.")
