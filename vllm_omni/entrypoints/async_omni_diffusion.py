...
from vllm_omni.diffusion.utils.wan_native import (
    ...
    load_wan22_native_t2v_transformer_config,
)
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if has_wan22_native_t2v_remote_layout(od_config.model):
                native_tf_cfg = load_wan22_native_t2v_transformer_config(od_config.model)
                if native_tf_cfg is None:
                    raise ValueError(
                        f"Detected WAN native T2V layout for {od_config.model}, "
                        "but cannot load high_noise_model/config.json"
                    )
                if od_config.model_class_name is None:
                    od_config.model_class_name = "WanPipeline"
                od_config.tf_model_config = TransformerConfig.from_dict(native_tf_cfg)
                od_config.update_multimodal_support()
                logger.info("Detected WAN native T2V checkpoint layout: %s", od_config.model)
            else:
